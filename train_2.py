...
def train(args, log_dir, writer, logger):
        ...
        aug = Compose(
            [
                RandomChoice(
                    [
                        RandomCropToSizeTorch(
                            data_format="dict", size=(args.image_size, args.image_size)
                        ),
                        ResizePaddedTorch(
                            (0, 0),
                            data_format="dict",
    ...
    # 21: number of heatmap channels
    # 12: number of room segmentation logits (multiclass over 12 room types)
    ...
    if args.arch == "hg_furukawa_original":
        ...
        # replace the last conv layer with a 1x1 conv layer to output the desired number of classes
        # 256: number of channels in the last feature map
        # args.n_classes: number of classes to output (multiclass over 44 room + icon types)
        # here is where we can re-configure the number of classes to output
    ...
    for epoch in range(start_epoch, args.n_epoch):
        ...
        # s is the learnable uncertainty parameter on UncertaintyLoss
        ss = pd.DataFrame()
        # Training
        ...
        avg_loss = np.inf  # BUG: why is this set to inf?
        loss = losses.mean()
        ...
        # we have two learning rates: one for the base model and one for the uncertainty parameter
        # can they diverge? (NB:they are initialized to the same value.)
        current_lr = {
        ...
        for i_val, samples_val in tqdm(
            ...
            with torch.no_grad():
                ...
                room_pred = (
                    ...
                    .argmax(0)
                    .data.cpu()  # .data seems to be an old-style way of getting the data from a tensor, use .cpu() instead ?
                    .numpy()
                )
        ...
        writer.add_scalars(
            "validation/room/IoU", room_class_iou["Class IoU"], global_step=1 + epoch
        )
        writer.add_scalars(
            "validation/room/Acc", room_class_iou["Class Acc"], global_step=1 + epoch
        )
        ...
        writer.add_scalars(
            "validation/icon/IoU", icon_class_iou["Class IoU"], global_step=1 + epoch
        )
        writer.add_scalars(
            "validation/icon/Acc", icon_class_iou["Class Acc"], global_step=1 + epoch
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-classes", nargs="?", type=int, default=44, help="# of the epochs"
    )
    parser.add_argument(
