...
class CubiCasa5KUnet(smp.Unet):
    ...
    def __init__(self, args, logger, n_output_channels=None):
...
class CubiCasa5KSegFormer(nn.Module):
    ...
    def __init__(self, args, logger, n_output_channels=None):
        ...
        n_labels = (
            ...
            else n_segmentation_classes(segmentation_map)
        )
...
class CubiCasa5KFurukawaWall(hg_furukawa_original):
    """Wall-point-only Furukawa head: raw logits (n_heatmap_channels=0), sigmoid
    applied externally by the trainer — same convention as train_simple's segmentation
    head, so all three architectures (Furukawa/U-Net/SegFormer) present raw logits
    uniformly to train_wall.py."""

    def __init__(self, args, logger):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.n_out = args.n_wall_channels
        n_heatmap_channels = 0
        logger.info(
            f"Using furukawa model with {self.n_out} channels for wall-point heatmaps"
            f" (raw logits; sigmoid applied by trainer)"
        )
...
def cubi_casa5k_wall_model(args, logger):
    _m = args.model
    n_channels = args.n_wall_channels
    if isinstance(_m, str) and _m.lower() == "segformer":
        return CubiCasa5KSegFormer(args, logger, n_output_channels=n_channels)
    if isinstance(_m, str) and _m.startswith("unet"):
        return CubiCasa5KUnet(args, logger, n_output_channels=n_channels)
    return CubiCasa5KFurukawaWall(args, logger)
