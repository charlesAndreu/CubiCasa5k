...
class House:
    def __init__(
        self, path, height, width, icon_list=icons_selected, room_list=rooms_selected
    ):
        self.height = height
        self.width = width
        shape = height, width
            ...
            if "Space " in e.getAttribute("class"):
                ...
                if len(rr) != 0:
                    ...
                    if len(rr) != 0 and len(cc) != 0:
                        ...
                        center_box = [
                            [rr_mean - 10, cc_mean - 10],
                            [rr_mean + 10, cc_mean + 10],
                        ]
                        room_name = (
                            e.getAttribute("class").replace("Space ", "").split(" ")[0]
                        )
                        room_name = room_name_map[room_name]
                        self.representation["labels"].append(
                            [center_box, [room_name, 1, 1]]
                        )
            ...
            for lineIndex_2, wall_2 in enumerate(walls):
                ...
                if minDistance <= lineWidth:
                    ...
                    if pointIndex_1 > -1 and pointIndex_2 > -1:
                        ...
                        side = [None, None]
                        side[lineDim_1] = (
                            line_1[1 - pointIndex_1][lineDim_1] - fixedValue_2
                        )
                        side[lineDim_2] = (
                            line_2[1 - pointIndex_2][lineDim_2] - fixedValue_1
                        )
                    ...
                    elif (pointIndex_1 > -1 and pointIndex_2 == -1) or (
                        pointIndex_1 == -1 and pointIndex_2 > -1
                    ):
        ...
        for lineIndex, pointMask in enumerate(usedLinePointMask):
            ...
            for pointIndex in range(2):
                if pointMask[pointIndex] is True:
                    continue
                point = [
                    lines[lineIndex][pointIndex][0],
                    lines[lineIndex][pointIndex][1],
                ]
                if pointIndex == 0:
                    if lineDim == 0:
        ...
        for i, point1 in enumerate(points):
            if merged[i] is False:
                ...
                for j, point2 in enumerate(points):
                    if (
                        j != i
                        and merged[j] is False
                        and self._are_close(point1[0], point2[0], wall_width)
                    ):
                        merged[j] = True
                        pool.append(point2)
    ...
    def get_lineDim(self, line, lineWidth):
        ...
        if (
            abs(line[0][0] - line[1][0]) > abs(line[0][1] - line[1][1])
            and abs(line[0][1] - line[1][1]) <= lineWidth
        ):
            return 0
        elif (
            abs(line[0][1] - line[1][1]) > abs(line[0][0] - line[1][0])
            and abs(line[0][0] - line[1][0]) <= lineWidth
        ):
            return 1
        else:
            return -1
    ...
    def findNearestJunctionPair(self, line_1, line_2, gap):
        ...
        if minDistance > gap:
            ...
            if lineDim_1 + lineDim_2 == 1:
                ...
                if (
                    line_2[0][lineDim_2] < fixedValue_1
                    and line_2[1][lineDim_2] > fixedValue_1
                ):
