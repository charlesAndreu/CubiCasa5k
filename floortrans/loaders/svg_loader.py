...
class FloorplanSVG(Dataset):
    ...
    def get_txt(self, index):
        ...
        # Combining them to one tensor
        label = torch.tensor(house.get_segmentation_tensor())
        heatmaps = house.get_heatmap_dict()
        coef_width = 1
        if self.original_size:
            fplan = cv2.imread(
                self.data_folder + self.folders[index] + self.org_image_file_name
            )
            fplan = cv2.cvtColor(fplan, cv2.COLOR_BGR2RGB)  # correct color channels
            ...
            label = label.squeeze(0).to(torch.uint8)

            coef_height = float(height_org) / float(height)
            coef_width = float(width_org) / float(width)
            for key, value in heatmaps.items():
                heatmaps[key] = [
                    (int(round(x * coef_width)), int(round(y * coef_height)))
                    for x, y in value
                ]

        img = torch.tensor(fplan, dtype=torch.uint8)
