...
class UncertaintyLoss(Module):
    ...
    def forward(self, input, target):
        ...
        self.loss_rooms_var = cross_entropy(
            input=rooms_pred * torch.exp(-self.log_vars[0]), target=rooms_target
        )
        self.loss_icons_var = cross_entropy(
            input=icons_pred * torch.exp(-self.log_vars[1]), target=icons_target
        )
        ...
        if self.mask:
            ...
            self.loss_heatmap_var, self.vars_sum, self.loss_heatmap = (
                self.homosced_heatmap_mse_loss_mask(
                    heatmap_pred, heatmap_target, heatmap_mask, self.log_vars_mse
                )
            )
        else:
