...
def polygon_intersection(
    x_min, x_max, y_min, y_max, x_min_label, x_max_label, y_min_label, y_max_label
):
    if (
        x_max > x_min_label
        and x_max_label > x_min
        and y_max > y_min_label
    ...
    for i, wall1 in enumerate(walls):
        ...
        for j in range(i + 1, len(walls)):
            ...
            if wall1_dim == wall2_dim:
                ...
                pred_area = np.sqrt(
                    (x_max_wall2 - x_min_wall2) ** 2 + (y_max_wall2 - y_min_wall2) ** 2
                )
    ...
    for i, t in enumerate(types):
        ...
        if t["type"] == "icon" and int(t["class"]) in opening_types:
            for j, tt in enumerate(types):
                if (
                    not (polygons[j] == polygons[i]).all()
                    and tt["type"] == "icon"
                    and int(tt["class"]) in opening_types
                ):
                    # Different opening
...
def get_polygons(predictions, threshold, all_opening_types):
    ...
    walls = np.empty([0, 4, 2], int)
    wall_layers = [2, 8]
    walls, wall_types, wall_points, wall_lines, wall_point_orientation_lines_map = (
        get_wall_polygon(
            wall_heatmaps,
            room_seg,
            threshold,
...
def get_opening_polygon(
    ...
    point_info = calc_point_info(
        door_points, gap, point_orientations, orientation_ranges, height, width, True
    )
    ...
    for line_index, line in enumerate(door_lines):
        ...
        for delta in range(int(abs(neighbor_point[line_dim] - point[line_dim]) + 1)):
            ...
            for type_index in range(num_door_types):
                door_evidence_sums[type_index] += label_map[door_offset + type_index][
                    min(max(intermediate_point[1], 0), height - 1)
                ][min(max(intermediate_point[0], 0), width - 1)]

        door_types.append(
            (line_index, np.argmax(door_evidence_sums), np.max(door_evidence_sums))
        )
...
def get_opening_types(opening_polygons, icons_seg, all_opening_classes):
    ...
    for pol in opening_polygons:
        ...
        opening_evidence_sums = icons_seg[
            all_opening_classes, y_1 : y_2 + 1, x_1 : x_2 + 1
        ].sum(axis=(1, 2))
...
def get_icon_polygon(
    ...
    gap = 10
    icons = find_icons(
        icon_points, gap, point_orientations, orientation_ranges, height, width, False
    )
...
def drop_long_walls(walls, wall_points):
    ...
    for wall1, wall2 in combinations(walls, 2):
        if (
            wall1 not in bad_walls
            and wall2 not in bad_walls
            and walls_same_corner(wall1, wall2, wall_points)
        ):
...
def split_validation(tensor, shape, split):
    ...
    heatmaps = (
        F.interpolate(heatmaps, size=shape, mode="bilinear", align_corners=False)
        .squeeze()
        .data.numpy()
    )
...
def extract_local_max(
    ...
    for point_index in range(num_points):
        ...
        if close_point_suppression:
            mask[
                max(y - gap, 0) : min(y + gap, height - 1),
                max(x - gap, 0) : min(x + gap, width - 1),
            ] = 0
...
def maximum_suppression(mask, x, y, heatmap_value_threshold):
    ...
    for delta in deltas:
        ...
        if neighbor_value <= value and neighbor_value > heatmap_value_threshold:
            maximum_suppression(mask, neighbor_x, neighbor_y, heatmap_value_threshold)
            pass
        continue
    ...
    for point_index, point in enumerate(points):
        ...
        for orientation in orientations:
            ...
            for neighbor_point_index, neighbor_point in enumerate(points):
                if (
                    neighbor_point_index <= point_index and not double_direction
                ...
                for c in range(2):
                    if (
                        neighbor_point[c] < ranges[c]
                        or neighbor_point[c] > ranges[c + 2]
                    ):
                        in_range = False
                        break
                    continue
                ...
                ) < max(abs(neighbor_point[1 - line_dim] - point[1 - line_dim]), 1):
                    continue
            ...
            for neighbor_point_index in neighbor_points:
                ...
                if double_direction and (
                    (point_index, neighbor_point_index) in lines
                    or (neighbor_point_index, point_index) in lines
                ):
                    continue

                line_index = len(lines)
                point_orientation_lines_map[point_index][orientation].append(line_index)
                ...
                if (
                    ...
                    < points[neighbor_point_index][0] + points[neighbor_point_index][1]
                ):
...
def draw_line_mask(points, lines, height, width, line_width=5, background_image=None):
    ...
    for line_index, line in enumerate(lines):
        ...
        fixed_value = int(round((point_1[1 - line_dim] + point_2[1 - line_dim]) / 2))
        min_value = int(min(point_1[line_dim], point_2[line_dim]))
        max_value = int(max(point_1[line_dim], point_2[line_dim]))
        if line_dim == 0:
            line_mask[
                max(fixed_value - line_width, 0) : min(
                    fixed_value + line_width, height
                ),
            ...
            line_mask[
                min_value : max_value + 1,
                max(fixed_value - line_width, 0) : min(fixed_value + line_width, width),
            ] = 1
            pass
        continue
    ...
    for line_index_1, line_1 in enumerate(lines):
        ...
        for line_index_2, line_2 in enumerate(lines):
            ...
            if line_dim_1 == line_dim_2:
                if (
                    abs(fixed_value_2 - fixed_value_1) > gap / 2
                    or min_value_1 > max_value_2 - gap
                    or min_value_2 > max_value_1 - gap
                ):
                    continue
                conflict_line_pairs.append((line_index_1, line_index_2))
                ...
                if (
                    min_value_1 > fixed_value_2 - gap
                    or max_value_1 < fixed_value_2 + gap
                    or min_value_2 > fixed_value_1 - gap
                    or max_value_2 < fixed_value_1 + gap
                ):
                    continue
                conflict_line_pairs.append((line_index_1, line_index_2))
                pass
...
def find_conflict_rectangle_pairs(points, rectangles, gap):
    ...
    for rectangle_index_1, rectangle_1 in enumerate(rectangles):
        for rectangle_index_2, rectangle_2 in enumerate(rectangles):
            ...
            min_x = max(
                points[rectangle_1[0]][0],
                points[rectangle_1[2]][0],
                points[rectangle_2[0]][0],
                points[rectangle_2[2]][0],
            )
            max_x = min(
                points[rectangle_1[1]][0],
                points[rectangle_1[3]][0],
                points[rectangle_2[1]][0],
                points[rectangle_2[3]][0],
            )
            if min_x > max_x - gap:
                continue
            min_y = max(
                points[rectangle_1[0]][1],
                points[rectangle_1[1]][1],
                points[rectangle_2[0]][1],
                points[rectangle_2[1]][1],
            )
            max_y = min(
                points[rectangle_1[2]][1],
                points[rectangle_1[3]][1],
                points[rectangle_2[2]][1],
                points[rectangle_2[3]][1],
            )
            if min_y > max_y - gap:
                continue
...
def find_icons(
    ...
    for point_index, point in enumerate(points):
        ...
        for orientation in orientations:
            ...
            for neighbor_point_index, neighbor_point in enumerate(points):
                ...
                if not in_range or abs(
                    neighbor_point[line_dim] - point[line_dim]
                ) < max(abs(neighbor_point[1 - line_dim] - point[1 - line_dim]), gap):
                    continue
    ...
    for point_index_1, orientation_neighbors in enumerate(
        ...
        if (
            ordered_orientations[0] not in orientation_neighbors
            or ((ordered_orientations[3] + 2) % 4) not in orientation_neighbors
        ):
            continue
        ...
        for point_index_2 in orientation_neighbors[ordered_orientations[0]]:
            if (
                ordered_orientations[1]
                not in point_orientation_neighbors_map[point_index_2]
            ):
                continue
            for point_index_3 in point_orientation_neighbors_map[point_index_2][
                ordered_orientations[1]
            ]:
                if (
                    ordered_orientations[2]
                    not in point_orientation_neighbors_map[point_index_3]
                ):
                    continue
                for point_index_4 in point_orientation_neighbors_map[point_index_3][
                    ordered_orientations[2]
                ]:
